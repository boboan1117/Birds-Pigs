	...
	local function Wallcollision(event)		
		if (owner.name =="wall" and event.other.name == "bird" )   then				
				...
				if (owner.health<1.5) then
					owner.fsm:changeState(sadwallState)
				end
...
function sadwallState:enter ( owner )
	...
	if ( owner.health<0.5) then
		...
		owner.fsm:changeState(deadwallState)
	end
end
...
function creatwall(x, y)
	...
	wall.name="wall"
	wall.health = 2
	wall.fsm = fsm.new(wall)
	wall.fsm:changeState(happywallState)
	return wall
end
		   
local wall1 = creatwall(300,280)
local wall2 = creatwall(365,280)
local wall3 = display.newImage("beam.png",335,220)
--wall3.name= "wall"
wall3.rotation = 90
group:insert(wall1)
group:insert(wall2)
...
function happypigState:enter (owner)
	local function Pigcollision(event)		
		if (owner.name =="pig" and event.other.name == "bird" ) or (owner.name == "pig" and  event.other.name == "wall") then				
				...
				if (owner.health<1.5) then
					owner.fsm:changeState(sadpigState)
...
function sadpigState:enter ( owner )
	...
	if ( owner.health<0.5) then
	owner.fsm:changeState(deadpigState)
	end
end
...
function makepig(x,y)
	...
	pig.name="pig"
	pig.fsm=fsm.new(pig)
	pig.fsm:changeState(happypigState)
	return pig
end	



local pig01=makepig(340,280)

physics.addBody(pig01)
pig01.name=("pig")
group:insert(pig01)
